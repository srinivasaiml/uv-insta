...
// .wrangler/tmp/bundle-iAxFeR/checked-fetch.js
...
// .wrangler/tmp/bundle-iAxFeR/middleware-loader.entry.ts
init_checked_fetch();
init_modules_watch_stub();

// .wrangler/tmp/bundle-iAxFeR/middleware-insertion-facade.js
init_checked_fetch();
init_modules_watch_stub();
    ...
    try {
      ...
      if (!video) {
        try {
          ...
          if (aRes.ok) {
            const text3 = await aRes.text();
            try {
              const aData = JSON.parse(text3);
              ...
              console.error("JSON Parse failed for API v1");
            }
          }
        } catch (e) {
...
function parseHtml(html3) {
  ...
  thumbnail = $2('meta[property="og:image"]').attr("content") || $2('meta[name="twitter:image"]').attr("content") || $2('meta[property="og:image:secure_url"]').attr("content") || "";
  return {
