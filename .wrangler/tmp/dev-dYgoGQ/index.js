...
// .wrangler/tmp/bundle-W4A42c/checked-fetch.js
...
// .wrangler/tmp/bundle-W4A42c/middleware-loader.entry.ts
init_checked_fetch();
init_modules_watch_stub();

// .wrangler/tmp/bundle-W4A42c/middleware-insertion-facade.js
init_checked_fetch();
init_modules_watch_stub();
...
var worker_default = {
  async fetch(request) {
    ...
    try {
      ...
      const fetchIG = /* @__PURE__ */ __name((url2, extraHeaders = {}) => fetch(url2, {
        headers: {
          "User-Agent": randomUA,
          "Accept": "*/*",
      ...
      try {
        ...
        const variables = JSON.stringify({ shortcode, child_comment_count: 3, fetch_comment_count: 40, parent_comment_count: 24, has_threaded_comments: false });
        const gRes = await fetchIG(`https://www.instagram.com/graphql/query/?query_hash=${queryHash}&variables=${encodeURIComponent(variables)}`, {
      ...
      if (!video) {
        try {
          const oRes = await fetchIG(`https://www.instagram.com/oembed/?url=${encodeURIComponent(cleanUrl)}`);
          if (oRes.ok) {
        ...
        throw new Error("Instagram is blocking extraction. This usually happens after many requests. Try again in 5 minutes.");
      }
      const finalVideo = video.replace(/\\u0026/g, "&").replace(/\\/g, "");
      const finalThumbnail = thumbnail.replace(/\\u0026/g, "&").replace(/\\/g, "");
      return new Response(JSON.stringify({ video: finalVideo, thumbnail: finalThumbnail }), {
